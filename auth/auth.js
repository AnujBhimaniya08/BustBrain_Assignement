...
router.get("/", (req, res) => {
  const latestRequestStateDisplayData =
    formatLatestTokenRequestStateForDeveloper();
  //   res.send({latestRequestStateDisplayData});
  res.send(`
    <div>
      <h3> New Token</h3>
      <a href="/auth/redirect-testing">Click to authorize and create a new access token</a>
      <br/>
      ...
      <form action="/auth/refresh_token" method="post" >
...
router.get("/redirect-testing", (req, res) => {
...
router.get("/airtable-oauth", (req, res) => {
  ...
  axios({
    ...
    .then(async (response) => {
      ...
      //   res.cookie("authToken", response.data.access)
...
router.get("/refresh_token_form", (req, res) => {
  const latestRequestStateDisplayData =
    formatLatestTokenRequestStateForDeveloper();
...
function setLatestTokenRequestState(state, dataToFormatIfExists) {
  ...
  if (dataToFormatIfExists) {
    ...
    const formattedData = json
      ...
      .map((line) =>
        line.replace(
          /^(\s+"(access_token|refresh_token)":)\s+"(.*)",$/g,
          "$1\n$3"
        )
      )
      .join("\n");
...
function formatLatestTokenRequestStateForDeveloper() {
  ...
  switch (latestTokenRequestState.state) {
    ...
    case "AUTHORIZATION_ERROR":
      formatRequestState =
        "Your latest authorization request failed, the error was:";
      break;
